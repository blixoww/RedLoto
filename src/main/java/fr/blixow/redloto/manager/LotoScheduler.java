...
public class LotoScheduler extends BukkitRunnable {
    public void run() {
        ...
        RedLoto.getLotoLogger().info(RedLoto.getLotoLogger().getName() + " Starting lotto pickup...");
        ...
        if (RedLoto.getInstance().haveAWinner()) {
            ...
            int gain = card.getCardValue() * RedLoto.getInstance().getLotoConfig().get().getInt("lotto.multiplier");
            ...
            RedLoto.getInstance().getVaultAPI().getEconomy().depositPlayer(player, (double) gain);
