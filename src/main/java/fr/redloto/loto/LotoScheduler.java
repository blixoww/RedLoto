...
public class LotoScheduler extends BukkitRunnable {
    ...
    public void run() {
        ...
        for (Player player : Bukkit.getOnlinePlayers())
            player.sendMessage("§7[§cRedLoto§7] Tirage en cours...");
        ...
        if (RedLoto.getInstance().haveAWinner()) {
            ...
            player.sendMessage("§7[§cRedLoto§7] Vous avez gagné ce tirage ! Gain de " + gain + " " + moneyName + ".");
            ...
            for (Player itPlayer : Bukkit.getOnlinePlayers())
                itPlayer.sendMessage("§7[§cRedLoto§7] Le tirage a été remporté par " + player.getDisplayName() + " ! §7Gain de §c" + gain + " " + moneyName + "§7.");
        } else
            for (Player player : Bukkit.getOnlinePlayers())
                player.sendMessage("§7[§cRedLoto§7] Le tirage n'a pas trouvé de gagnant pour cette edition !");
        RedLoto.getInstance().clearSession();
        int delay = RedLoto.getInstance().getLotoConfig().get().getInt("loto.delay");
        if (RedLoto.getInstance().getLotoStartingDelay() != null)
            RedLoto.lotoStartingDelay = new LotoStartingDelay();
        if (RedLoto.getInstance().getLotoStartingDelay() != null)
            RedLoto.getInstance().getLotoStartingDelay().runTaskLaterAsynchronously(RedLoto.getInstance(), delay * 20L);
