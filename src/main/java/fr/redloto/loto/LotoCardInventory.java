...
public class LotoCardInventory {
    ...
    public LotoCardInventory(LotoCard card) {
        this.inventory = Bukkit.createInventory(null, 27, "Loto Carte :" + card.getID());
    ...
    private void setupInventory(LotoCard card) {
        for (int i = 0; i < 27; i++) {
            if (i == 11)
                this.inventory.setItem(i, createItem("§c-", "§7Retirer 100 " + moneyName + ".", Material.WOOL, (short) 14, 1));
            else if (i == 13)
                this.inventory.setItem(i, createItem("§7Valeur", "§7Valeur de " + card.getCardValue() + " " + moneyName + ".", Material.PAPER, 1));
            else if (i == 15)
                this.inventory.setItem(i, createItem("§a+", "§7Ajouter 100 " + moneyName + ".", Material.WOOL, (short) 5, 1));
            else if (i == 25)
                this.inventory.setItem(i, createItem("§4Retour", "§7Revenir au menu des cartes.", Material.SIGN, 1));
            else if (i == 26)
                this.inventory.setItem(i, createItem("§4Fermer", "§7Fermer le menu lotto.", Material.BARRIER, 1));
            else
    ...
    public void updateInventory(LotoCard card) {
        this.inventory.setItem(13, createItem("§7Valeur", "§7Valeur de " + card.getCardValue() + " " + moneyName + ".", Material.PAPER, 1));
